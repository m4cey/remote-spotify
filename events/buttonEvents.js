...
buttons.joinButton = async (interaction) => {
  ...
  if (methods.isListener(userId)) {
    const message = methods.newMessage(
  ...
  } else interaction.followUp(methods.loginMessage(interaction));
};
...
buttons.playButton = async (interaction) => {
  ...
  for (user of listening) {
    try {
      ...
      if (!token) throw "No token provided";
      spotifyApi.setAccessToken(token);
      if (is_playing) methods.validateResponse(await spotifyApi.pause());
      else methods.validateResponse(await spotifyApi.play());
    } catch (error) {
      logger.error(error);
    } finally {
      spotifyApi.resetAccessToken();
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    logger.debug(
      `${interaction.user.tag} in #${interaction.channel.name} triggered a button: ${interaction.customId}`
    );
    try {
